<%@ Page Language="C#" MasterPageFile="~/Parent.master" AutoEventWireup="true" CodeFile="ActionTrackerSearch.aspx.cs" Inherits="ActionTrackerSearch" Title="Action Tracker Search" %>
...
$(document).ready(function(){
    $("#<%=lstColumn.ClientID %>").change(function(){
        ...
        {$("#<%=lblHelp.ClientID %>").text('Enter Category');}
        else if (fld=='SubCategory')
        {$("#<%=lblHelp.ClientID %>").text('Enter File Number / Others Name');}
        else if (fld=='OtherSubCategory')
        {$("#<%=lblHelp.ClientID %>").text('Enter Other Name');}
        else if (fld=='Action')
        {$("#<%=lblHelp.ClientID %>").text('Enter Action Description');}
        else if (fld=='ActionDt')
        {$("#<%=lblHelp.ClientID %>").text('Enter Action Date (DD/MM/YYYY)');}
        else if (fld=='ActionUser')
        {$("#<%=lblHelp.ClientID %>").text('Enter Action User');}
        else if (fld=='NextAction')
        {$("#<%=lblHelp.ClientID %>").text('Enter Next Action Description');}
        else if (fld=='FollowupDt')
        {$("#<%=lblHelp.ClientID %>").text('Enter Follow up date (DD/MM/YYYY)');}
        else if (fld=='AssignUser')
        {$("#<%=lblHelp.ClientID %>").text('Enter Assign User');}
        else if (fld=='ActionComplete')
        {$("#<%=lblHelp.ClientID %>").text('Enter Action complete(Yes or No)');}
        else if (fld=='Comments')
        {$("#<%=lblHelp.ClientID %>").text('Enter Comments');}        
    });    
  });
</script>
</asp:Content>
