...
public partial class Default2 : System.Web.UI.Page
{
    ...
    protected void btnFind_Click(object sender, EventArgs e)
    {
        ...
        string sql2 = "select SLNo,FileNo,AmtInFC,AmtInINR,Format(DueDate,'dd-MM-yyyy')as DueDate,Format(DueDate_Restoration,'dd-MM-yyyy') as DueDate_Restoration,Responsibility,Sharing_Party,Format(Intimation_Date,'dd-MM-yyyy') as Intimation_Date,Format(Confirmation_Date,'dd-MM-yyyy') as Confirmation_Date,Share_Received,Format(POPaymentDate,'dd-MM-yyyy') as PoPaymentDate,Status,Description from RenewalFollowup where FileNo='" + ddlIDFNo.SelectedItem.ToString() + "'";
    ...
    protected void BindAction(string ContractNo, int Slno)
    {
        ...
        string sql2 = "select SLNo,AmtInFC,AmtInINR,Format(DueDate,'dd-MM-yyyy')as DueDate,Amt_InCond,Format(DueDate_Restoration,'dd-MM-yyyy') as DueDate_Restoration ,Responsibility,Format(Intimation_Date,'dd-MM-yyyy') as Intimation_Date,Format(Confirmation_Date,'dd-MM-yyyy') as Confirmation_Date,Format(POPaymentDate,'dd-MM-yyyy') as POPaymentDate,Status,Sharing_Party,Share_Received, Percentage,Description from RenewalFollowup where FileNo='" + ContractNo + "' and SLNo='" + Slno + "'";
        cnp.Open();
        SqlCommand cmd = new SqlCommand();
